...
namespace aeplanner {
  ...
  void recurse(RRTNode * node, visualization_msgs::MarkerArray * marker_array, int * id, double lambda){
    for(std::vector<RRTNode*>::iterator child_it  = node->children_.begin();
      ...
      marker_array->markers.push_back(createEdgeMarker(child, (*id), "mavros/world", lambda)); // FIXME read frame id from config
      marker_array->markers.push_back(createNodeMarker(child, (*id)++, "mavros/world"));
    }
  }
